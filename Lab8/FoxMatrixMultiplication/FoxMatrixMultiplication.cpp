...
void broadcastBlockA(int iteration, double* block, double* initial_block) {
	int source = (process_coordinates[0] + iteration) % grid_size;

	if (process_coordinates[1] == source) {
		for (int i = 0; i < block_size * block_size; i++) {
			block[i] = initial_block[i];
		}
	}

	MPI_Bcast(block, block_size * block_size, MPI_DOUBLE, source, row_communicator);
}

// cyclic upward shift of matrix B blocks in the process grid columns
void shiftBlockB(double* block) {
	MPI_Status status;
	int next = (process_coordinates[0] != 0) ? (process_coordinates[0] - 1) : (grid_size - 1);
	int prev = (process_coordinates[0] + 1) % grid_size;

	MPI_Sendrecv_replace(block, block_size * block_size, MPI_DOUBLE, next, 0, prev, 0, column_communicator, &status);
}
...
int main(int argc, char* argv[]) {
	...
	if (num_of_processors != grid_size * grid_size) {
		...
		if (processor_rank == 0) {
			printf_s("Fox's matrix multiplication\n");
		...
		initial_block_a = new double[block_size * block_size];
		...
		for (int i = 0; i < block_size * block_size; i++) {
			initial_block_a[i] = block_a[i];
		}
		
		for (int i = 0; i < grid_size; i++) {
			broadcastBlockA(i, block_a, initial_block_a);
